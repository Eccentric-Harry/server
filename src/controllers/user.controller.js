...
const getUserChannelProfile = asyncHandler( async(req, res) => {
    ...
    const channel = await User.aggregate([   // aggregate use karne se hame Arrays return hote hai!
        {
            $match: {
                username: username?.toLowerCase()
            }
        ...
        },
        {
            $addFields : {
                ...
                isSubscribed : {
                    $if: { $in: [req.user?._id, "$subscribers.subscriber"] },  // if the user is in the subscribers array then :true,
                    then :true,
                    else: false
                }
            }
